...
// e-mails que recebem acesso automático em cada planilha nova criada
// (edite essa lista antes de rodar - adicione um e-mail por linha)
const EMAILS_PARA_COMPARTILHAR = [
  'ia.essenciadobrasil@gmail.com'
];
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem('🔗 Compartilhar arquivos já criados na pasta', 'compartilharArquivosExistentes')
    .addToUi();
}

/**
 * Passa por TODOS os arquivos que já estão na pasta de destino (inclusive
 * os criados antes dessa correção) e compartilha com os e-mails da lista
 * EMAILS_PARA_COMPARTILHAR. Use isso uma vez pra corrigir os arquivos que
 * já foram gerados sem compartilhamento automático.
 */
function compartilharArquivosExistentes() {
  ...
  while (arquivos.hasNext()) {
    ...
    total++;
    EMAILS_PARA_COMPARTILHAR.forEach(function(email) {
      try {
        arquivo.addViewer(email);
      } catch (e) {
        comErro++;
      }
    });
  ...
  SpreadsheetApp.getUi().alert('Compartilhamento concluído: ' + total + ' arquivo(s) processado(s)' +
    (comErro > 0 ? ', ' + comErro + ' erro(s) ao compartilhar (confira se os e-mails estão corretos)' : '.'));
}
  ...
  while (linhaAtual <= ultimaLinhaDados && (Date.now() - inicio) < TEMPO_LIMITE_MS) {
    ...
    while (tentativas < 3 && !sucesso) {
      ...
      try {
        ...
        // dados: o objeto de imagem (CellImage) NÃO pode ser transferido
        // direto pra outro arquivo (testamos e dá erro) - por isso tiramos
        // ele da coluna F antes de escrever, e recriamos a imagem depois
        // a partir do link que está na coluna E.
        const valoresLote = origem.getRange(linhaAtual, 1, numLinhas, NUM_COLUNAS).getValues();
        for (let i = 0; i < valoresLote.length; i++) {
          valoresLote[i][COL_IMAGEM - 1] = '';
        }
        novaAba.getRange(2, 1, numLinhas, NUM_COLUNAS).setValues(valoresLote);

        // recria a imagem a partir do link, direto no arquivo novo
        for (let i = 0; i < numLinhas; i++) {
          const link = valoresLote[i][COL_LINK_CAPA - 1];
          if (link) {
            try {
              const imagemCelula = SpreadsheetApp.newCellImage().setSourceUrl(link).build();
              novaAba.getRange(2 + i, COL_IMAGEM).setValue(imagemCelula);
            } catch (eImg) {
              logSheet.appendRow([new Date(), 'linha ' + (linhaAtual + i), 'Erro ao recriar imagem: ' + detalheErro(eImg), '']);
            }
          }
        }
        ...
        // compartilha automaticamente com os e-mails configurados
        EMAILS_PARA_COMPARTILHAR.forEach(function(email) {
          try {
            arquivoDrive.addViewer(email);
          } catch (eShare) {
            logSheet.appendRow([new Date(), pad4(linhaAtual) + '-' + pad4(linhaFim),
              'Aviso: não consegui compartilhar com ' + email + ' - ' + detalheErro(eShare), '']);
          }
        });
