...
function processarLoteDivisao() {
  ...
  while (linhaAtual <= ultimaLinhaDados && (Date.now() - inicio) < TEMPO_LIMITE_MS) {
    ...
    try {
      ...
      // dados + fotos (copyTo preserva as imagens na célula)
      const origemRange = origem.getRange(linhaAtual, 1, numLinhas, NUM_COLUNAS);
      const destinoRange = novaAba.getRange(2, 1, numLinhas, NUM_COLUNAS);
      origemRange.copyTo(destinoRange);
      ...
      logSheet.appendRow([new Date(), pad4(linhaAtual) + '-' + pad4(linhaFim), 'ERRO: ' + e.message, '']);
    }
