 ...
 *                                 da foto | Preço | Nome). Já existente.
 ...
 *                                 que a IA preenche por tema). Uma célula
 *                                 "Texto pronto pra copiar" já junta tudo
 *                                 automaticamente num bloco só.
 * 4. "Marketing em Preparação" - estrutura FIXA (serve pra qualquer
 *                                 template), reaproveitada a cada campanha:
 *                                 5 campos de nome de produto (buscados em
 *                                 "Links Produtos") + 1 campo de texto único
 *                                 (onde se cola o bloco copiado da
 *                                 "Templade").
 *
 ...
 * 4. Você copia a célula "Texto pronto pra copiar" e cola no campo de texto
 *    da aba "Marketing em Preparação", e preenche os nomes dos produtos.
 * 5. Você roda, no menu, o envio daquele template específico (ex:
 *    "Enviar - Template A"). O script confere se o nome do template dentro
 *    do texto colado bate com o que você rodou, busca os produtos por nome,
 *    monta o HTML e envia.
 *
 * CADA TEMPLATE NOVO = 1 função de montagem (ex: montarHtmlTemplateA) + 1
 * função de envio (ex: enviarTemplateA) + 1 item de menu novo. Quando você
 * criar um "Template B" na aba "Templade", é só pedir que eu adiciono o
 * código dele aqui do mesmo jeito.
 *
 ...
 * 6. Rode "🧩 Criar aba 'Templade'" e "📝 Criar aba 'Marketing em
 *    Preparação'" pra criar as duas abas novas (só precisa rodar uma vez -
 *    se a aba já existir, o script avisa e não sobrescreve).
 */
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem('✉️ Enviar - Template A', 'enviarTemplateA')
    .addToUi();
}
...
function criarAbaTemplade() {
  ...
  // Junta as linhas 14 a 26 (NOME DO TEMPLATE até RODAPÉ TEXTO) em um
  // bloco de texto único, pronto pra copiar - recalcula sozinho se você
  // editar qualquer valor da ficha técnica.
  sheet.getRange(28, 2).setFormula('=TEXTJOIN(CHAR(10),TRUE,ARRAYFORMULA(A14:A26&": "&B14:B26))');
 ...
 * fixa, reaproveitada a cada campanha - 5 campos de nome de produto e 1
 * campo de texto único (onde se cola o bloco copiado da "Templade").
 */
function criarAbaMarketingPreparacao() {
  ...
  sheet.setRowHeight(7, 220);
  sheet.setColumnWidth(1, 320);
  sheet.setColumnWidth(2, 420);
  sheet.setFrozenRows(1);

  SpreadsheetApp.getUi().alert('Aba "' + ABA_PREPARACAO + '" criada. Preencha os nomes dos produtos (linhas 2 a 6) e cole o texto da ficha técnica na linha 7, depois rode o envio do template correspondente no menu.');
}
 ...
 * nem espaços nas pontas) na aba "Links Produtos". Retorna null se não
 * encontrar.
 */
function buscarProdutoPorNome(nome) {
  ...
  for (let i = 0; i < dados.length; i++) {
    const [link, foto, preco, nomeProduto] = dados[i];
    if (String(nomeProduto).trim().toLowerCase() === alvo) {
      return { link: link || '#', foto: foto || '', preco: preco, nome: nomeProduto };
    }
  }
  return null;
}
...
/**
 * Lê a aba "Marketing em Preparação": 5 nomes de produto (linhas 2 a 6,
 * coluna B) + o texto único colado (linha 7, coluna B).
 */
function lerPreparacao() {
  ...
  const produtos = sheet.getRange(2, 2, 5, 1).getValues().map(function(linha) {
    return String(linha[0] || '').trim();
  });
  const texto = sheet.getRange(7, 2).getValue();

  return { produtos: produtos, texto: String(texto || '') };
}
...
function enviarTemplateA() {
  ...
  if (!preparacao.texto.trim()) {
    ui.alert('O campo de texto da aba "' + ABA_PREPARACAO + '" está vazio. Copie a "Ficha técnica" do Template A na aba "Templade" e cole ali antes de enviar.');
    return;
  }

  if (nomeTemplateNoTexto !== 'template a') {
    ui.alert('O texto colado diz "NOME DO TEMPLATE: ' + (campos['NOME DO TEMPLATE'] || '(vazio)') + '", mas você rodou o envio do Template A. Confira se colou a ficha técnica certa antes de enviar.');
    return;
  }

  const produtos = [];
  for (let i = 0; i < preparacao.produtos.length; i++) {
    const nome = preparacao.produtos[i];
    if (!nome) {
      produtos.push(null);
      continue;
    }
    ...
    if (!produto) {
      ui.alert('Não encontrei o produto "' + nome + '" na aba "' + ABA_PRODUTOS + '" (Produto ' + (i + 1) + '). Confira se o nome está exatamente igual e tente de novo.');
      return;
    }
    produtos.push(produto);
  }

  const html = montarHtmlTemplateA(produtos, campos);
  ...
  ui.alert('E-mail (Template A) enviado para ' + EMAIL_DESTINO_TESTE + '. Confira sua caixa de entrada.');
}
