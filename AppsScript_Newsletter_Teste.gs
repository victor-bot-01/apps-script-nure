 ...
 *    campo de texto da aba "Marketing em Preparação", e preenche os nomes
 *    dos produtos.
...
function atualizarTextoParaCopiar() {
  ...
  const inicio = dados.findIndex(function(linha) { return String(linha[0]).trim().toUpperCase() === 'NOME DO TEMPLATE'; });
  const fim = dados.findIndex(function(linha) { return String(linha[0]).trim().toUpperCase() === 'RODAPÉ TEXTO'; });
  const linhaAlvo = dados.findIndex(function(linha) { return String(linha[0]).toLowerCase().indexOf('texto pronto pra copiar') !== -1; });
