...
const TEXTO_PADRAO_PEDIR_BANNER_HERO =
  ...
  'estar prontas em outro campo da planilha).\n' +
  '\n' +
...
function criarAbaTemplade() {
  ...
  sheet.getRange('A23').setFontWeight('bold');
  sheet.getRange('A43').setFontWeight('bold');
  ...
  sheet.setRowHeight(18, 460);
  sheet.setRowHeight(19, 460);
  sheet.setRowHeight(20, 460);
  sheet.setRowHeight(21, 460);
  sheet.setRowHeight(32, 260);
...
function atualizarTextosComProdutos() {
  ...
  if (idxLinks !== -1) {
    const linhasLinks = nomesProdutos.map(function(nome) {
      const produto = buscarProdutoPorNome(nome);
      return nome + ': ' + (produto && produto.foto ? produto.foto : '(foto não encontrada em "' + ABA_PRODUTOS + '")');
    });
    sheet.getRange(idxLinks + 1, 2).setValue(linhasLinks.join('\n'));
  }

  ui.alert('Textos e links atualizados com os produtos do campo "' + NOME_LINHA_PRODUTOS_FICHA + '". Confira as linhas na aba "' + ABA_TEMPLADE + '".');
}
