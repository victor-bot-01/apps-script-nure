 ...
 *                                 até 5 linhas de produto (nome, preço
 *                                 pesquisado, preço com desconto e
 *                                 observação) + 1 campo de texto único
 *                                 (onde se cola o bloco copiado da
 *                                 "Templade"). O "preço pesquisado" é
 *                                 conferido contra o preço já cadastrado em
 *                                 "Links Produtos" antes de enviar - se
 *                                 forem diferentes, o script avisa e NÃO
 *                                 envia (o preço da aba de produtos pode
 *                                 estar desatualizado).
 *
...
function limparMarketingPreparacao() {
  ...
  sheet.getRange(2, 2, 5, 4).clearContent();
  sheet.getRange(8, 2).clearContent();
 ...
 * fixa, reaproveitada a cada campanha - uma linha por produto (nome, preço
 * pesquisado, preço com desconto, observação) + 1 campo de texto único
 * (onde se cola o bloco copiado da "Templade").
 */
function criarAbaMarketingPreparacao() {
  ...
  const linhas = [
    ['Produto', 'Nome', 'Preço pesquisado', 'Preço com desconto', 'Observação'],
    ['Produto 1', '', '', '', ''],
    ['Produto 2', '', '', '', ''],
    ['Produto 3', '', '', '', ''],
    ['Produto 4', '', '', '', ''],
  ...
  sheet.getRange(8, 1).setValue('Texto (colar aqui a ficha técnica copiada da aba "Templade")');
  sheet.getRange(8, 1).setWrap(true);
  sheet.getRange(8, 2).setWrap(true);
  sheet.setRowHeight(8, 220);

  SpreadsheetApp.getUi().alert('Aba "' + ABA_PREPARACAO + '" criada. Preencha o nome (obrigatório), o preço pesquisado (obrigatório - vai ser conferido com o preço já cadastrado em "Links Produtos"), o preço com desconto e a observação (opcionais) de cada produto nas linhas 2 a 6, e cole o texto da ficha técnica na linha 8, depois rode o envio do template correspondente no menu.');
}
...
/**
 * Lê a aba "Marketing em Preparação": até 5 produtos (linhas 2 a 6 - nome,
 * preço pesquisado, preço com desconto e observação nas colunas B a E) +
 * o texto único colado (linha 8, coluna B).
 */
function lerPreparacao() {
  ...
  const produtos = sheet.getRange(2, 2, 5, 4).getValues().map(function(linha) {
    return {
      nome: String(linha[0] || '').trim(),
      precoPesquisado: linha[1],
      precoComDesconto: linha[2],
      observacao: String(linha[3] || '').trim(),
    };
  });
  const texto = String(sheet.getRange(8, 2).getValue() || '').trim();
...
function enviarTemplate() {
  ...
  for (let i = 0; i < preparacao.produtos.length; i++) {
    ...
    if (precoPesquisado === null) {
      ui.alert('Falta o "Preço pesquisado" do Produto ' + (i + 1) + ' ("' + entrada.nome + '") na aba "' + ABA_PREPARACAO + '". Preencha antes de enviar.');
      return;
    }

    const precoCatalogo = paraNumero(produtoCatalogo.preco);
    if (precoCatalogo === null || Math.round(precoCatalogo * 100) !== Math.round(precoPesquisado * 100)) {
      ui.alert(
        'O preço do Produto ' + (i + 1) + ' ("' + entrada.nome + '") está diferente entre as abas: "' + ABA_PRODUTOS + '" tem ' +
        (precoCatalogo === null ? '(vazio/inválido)' : formatarPreco(precoCatalogo)) + ' e "' + ABA_PREPARACAO + '" tem ' +
        formatarPreco(precoPesquisado) + '. Confira qual está certo antes de enviar - o e-mail NÃO foi enviado.'
      );
      return;
    }
