...
// grande cabe pouco, botão cabe bem menos, parágrafo cabe mais).
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem('📋 Copiar ficha técnica pronta', 'copiarFichaTecnica')
...
/**
 * Item de menu: abre o "Texto pronto pra pedir produtos" (aba "Templade")
 * pronto pra copiar.
 */
function copiarTextoPedirProdutos() {
  ...
  const dados = sheet.getRange(1, 1, sheet.getLastRow(), 2).getValues();
  const linhaIdx = dados.findIndex(function(linha) { return String(linha[0]).trim() === NOME_LINHA_TEXTO_PEDIR_PRODUTOS; });
  if (linhaIdx === -1) {
    ui.alert('Não encontrei a linha "' + NOME_LINHA_TEXTO_PEDIR_PRODUTOS + '" na aba "' + ABA_TEMPLADE + '". Rode primeiro "🔗 Adicionar \'Texto pronto pra pedir produtos\' (Templade)".');
    return;
  }

  mostrarDialogoCopiar(dados[linhaIdx][1], 'Texto pronto pra pedir produtos');
}
...
/**
 * Adiciona a linha "Texto pronto pra pedir produtos" na seção "Estrutura"
 * da aba "Templade", pra quem já criou a aba antes dessa linha existir.
 * Não faz nada se a linha já existir (idempotente - pode rodar de novo à
 * vontade).
 */
function adicionarTextoParaPedirProdutos() {
  ...
  const jaExiste = dados.some(function(linha) { return String(linha[0]).trim() === NOME_LINHA_TEXTO_PEDIR_PRODUTOS; });
  if (jaExiste) {
    ui.alert('Já existe uma linha "' + NOME_LINHA_TEXTO_PEDIR_PRODUTOS + '" na aba "' + ABA_TEMPLADE + '".');
    return;
  }

  const linhaFuncaoEnvioIdx = dados.findIndex(function(linha) { return String(linha[0]).trim() === 'Função de envio'; });
  const linhaSheetInsercao = linhaFuncaoEnvioIdx === -1 ? sheet.getLastRow() : linhaFuncaoEnvioIdx + 1;

  sheet.insertRowAfter(linhaSheetInsercao);
  const novaLinha = linhaSheetInsercao + 1;
  sheet.getRange(novaLinha, 1).setValue(NOME_LINHA_TEXTO_PEDIR_PRODUTOS).setWrap(true);
  sheet.getRange(novaLinha, 2).setValue(TEXTO_PADRAO_PEDIR_PRODUTOS).setWrap(true);
  sheet.setRowHeight(novaLinha, 170);

  ui.alert('Pronto! Veja a linha "' + NOME_LINHA_TEXTO_PEDIR_PRODUTOS + '" na aba "' + ABA_TEMPLADE + '".');
}
